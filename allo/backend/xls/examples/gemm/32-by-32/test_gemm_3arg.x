// Test suite for 32x32 GEMM implementations (3-argument version)
...
fn zero_matrix() -> u32[32][32] {
  u32[32][32]:[[u32:0, ...], ...]
}

// Helper function to create identity matrix
fn identity_matrix() -> u32[32][32] {
...
fn test_zero_multiplication() {
  // A * 0 + 0 = 0 for any matrix A
  let A = simple_matrix();
  let zero = zero_matrix();
  let result = gemm(A, zero, zero);
...
fn test_identity_multiplication() {
  // A * I + 0 = A for any matrix A
  let A = simple_matrix();
  let I = identity_matrix();
  let zero = zero_matrix();
  let result = gemm(A, I, zero);
