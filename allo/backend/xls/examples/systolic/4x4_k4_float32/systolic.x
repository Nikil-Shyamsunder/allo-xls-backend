...
pub proc SystolicArray {
  ...
  next(state: (float32::F32[4][4], float32::F32[4][4], u32, bool)) {
    ...
    let tok2 = tok_b3;
    let _ = recv_non_blocking(token(), from_hor[u32:0][u32:4], float32::zero(u1:0));
    let _ = recv_non_blocking(token(), from_hor[u32:1][u32:4], float32::zero(u1:0));
    let _ = recv_non_blocking(token(), from_hor[u32:2][u32:4], float32::zero(u1:0));
    let _ = recv_non_blocking(token(), from_hor[u32:3][u32:4], float32::zero(u1:0));
    let _ = recv_non_blocking(token(), from_vert[u32:4][u32:0], float32::zero(u1:0));
    let _ = recv_non_blocking(token(), from_vert[u32:4][u32:1], float32::zero(u1:0));
    let _ = recv_non_blocking(token(), from_vert[u32:4][u32:2], float32::zero(u1:0));
    let _ = recv_non_blocking(token(), from_vert[u32:4][u32:3], float32::zero(u1:0));
