...
pub proc SystolicArray {
  ...
  config(
    ...
    (a_row_0, a_row_1, b_col_0, b_col_1, b_col_2, b_col_3, c_result)
  }
  ...
  next(state: ()) {
    ...
    let (a_fifo_0_3_s, a_fifo_0_3_r) = chan<u32>("a_fifo_0_3");
    let (a_fifo_0_4_s, a_fifo_0_4_r) = chan<u32>("a_fifo_0_4");
    ...
    let (a_fifo_1_3_s, a_fifo_1_3_r) = chan<u32>("a_fifo_1_3");
    let (a_fifo_1_4_s, a_fifo_1_4_r) = chan<u32>("a_fifo_1_4");
    ...
    let (b_fifo_2_0_s, b_fifo_2_0_r) = chan<u32>("b_fifo_2_0");
    let (b_fifo_2_1_s, b_fifo_2_1_r) = chan<u32>("b_fifo_2_1");
    let (b_fifo_2_2_s, b_fifo_2_2_r) = chan<u32>("b_fifo_2_2");
    let (b_fifo_3_0_s, b_fifo_3_0_r) = chan<u32>("b_fifo_3_0");
    let (b_fifo_3_1_s, b_fifo_3_1_r) = chan<u32>("b_fifo_3_1");
    let (b_fifo_3_2_s, b_fifo_3_2_r) = chan<u32>("b_fifo_3_2");
    let (c_out_0_0_s, c_out_0_0_r) = chan<u32>("c_out_0_0");
    let (c_out_0_1_s, c_out_0_1_r) = chan<u32>("c_out_0_1");
    let (c_out_0_2_s, c_out_0_2_r) = chan<u32>("c_out_0_2");
    let (c_out_0_3_s, c_out_0_3_r) = chan<u32>("c_out_0_3");
    let (c_out_1_0_s, c_out_1_0_r) = chan<u32>("c_out_1_0");
    let (c_out_1_1_s, c_out_1_1_r) = chan<u32>("c_out_1_1");
    let (c_out_1_2_s, c_out_1_2_r) = chan<u32>("c_out_1_2");
    let (c_out_1_3_s, c_out_1_3_r) = chan<u32>("c_out_1_3");
    ...
    spawn PE<u32:4>(a_fifo_0_2_r, b_fifo_2_0_r, a_fifo_0_3_s, b_fifo_2_1_s, c_out_0_2_s);
    spawn PE<u32:4>(a_fifo_0_3_r, b_fifo_3_0_r, a_fifo_0_4_s, b_fifo_3_1_s, c_out_0_3_s);
    ...
    spawn PE<u32:4>(a_fifo_1_2_r, b_fifo_2_1_r, a_fifo_1_3_s, b_fifo_2_2_s, c_out_1_2_s);
    spawn PE<u32:4>(a_fifo_1_3_r, b_fifo_3_1_r, a_fifo_1_4_s, b_fifo_3_2_s, c_out_1_3_s);
    let tok = join();
    ...
    let (tok, b_0_3) = recv(tok, b_col_3);
    let tok = send(tok, b_fifo_3_0_s, b_0_3);
    let (tok, b_1_3) = recv(tok, b_col_3);
    let tok = send(tok, b_fifo_3_0_s, b_1_3);
    let (tok, b_2_3) = recv(tok, b_col_3);
    let tok = send(tok, b_fifo_3_0_s, b_2_3);
    let (tok, b_3_3) = recv(tok, b_col_3);
    let tok = send(tok, b_fifo_3_0_s, b_3_3);
    let (tok, c_0_0) = recv(tok, c_out_0_0_r);
    let (tok, c_0_1) = recv(tok, c_out_0_1_r);
    let (tok, c_0_2) = recv(tok, c_out_0_2_r);
    let (tok, c_0_3) = recv(tok, c_out_0_3_r);
    let (tok, c_1_0) = recv(tok, c_out_1_0_r);
    let (tok, c_1_1) = recv(tok, c_out_1_1_r);
    let (tok, c_1_2) = recv(tok, c_out_1_2_r);
    let (tok, c_1_3) = recv(tok, c_out_1_3_r);
    let (tok, _) = recv(tok, a_fifo_0_4_r);
    let (tok, _) = recv(tok, a_fifo_0_4_r);
    let (tok, _) = recv(tok, a_fifo_0_4_r);
    let (tok, _) = recv(tok, a_fifo_0_4_r);
    let (tok, _) = recv(tok, a_fifo_1_4_r);
    let (tok, _) = recv(tok, a_fifo_1_4_r);
    let (tok, _) = recv(tok, a_fifo_1_4_r);
    let (tok, _) = recv(tok, a_fifo_1_4_r);
    ...
    let (tok, _) = recv(tok, b_fifo_1_2_r);
    let (tok, _) = recv(tok, b_fifo_1_2_r);
    let (tok, _) = recv(tok, b_fifo_2_2_r);
    let (tok, _) = recv(tok, b_fifo_2_2_r);
    let (tok, _) = recv(tok, b_fifo_2_2_r);
    let (tok, _) = recv(tok, b_fifo_2_2_r);
    let (tok, _) = recv(tok, b_fifo_3_2_r);
    let (tok, _) = recv(tok, b_fifo_3_2_r);
    let (tok, _) = recv(tok, b_fifo_3_2_r);
    let (tok, _) = recv(tok, b_fifo_3_2_r);
    let C = u32[2][4]:[[c_0_0, c_0_1, c_0_2, c_0_3], [c_1_0, c_1_1, c_1_2, c_1_3]];
