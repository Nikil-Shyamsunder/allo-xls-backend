#!/usr/bin/env python3
"""
MLIR to XLS IR Function Conversion Script

This script takes raw MLIR text as input and produces XLS IR output
by directly lowering to IR (bypassing DSLX).

Usage:
  # From a file:
  ./mlir_to_ir_fn.py input.mlir -o output.ir

  # From stdin:
  cat input.mlir | ./mlir_to_ir_fn.py --stdin -o output.ir

  # Print to stdout:
  ./mlir_to_ir_fn.py input.mlir
"""
...
# Script is at: allo/backend/xls/scripts/mlir_to_ir_fn.py
# Need to go up 4 levels to reach the root
...
def mlir_to_ir(mlir_module):
    """Convert MLIR module to XLS IR code."""
    try:
        ...
        raise RuntimeError(f"Failed to convert MLIR to XLS IR: {e}")


def main():
    parser = argparse.ArgumentParser(
        description="Convert MLIR to XLS IR (function-based)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s input.mlir -o output.ir
  %(prog)s input.mlir -o output_dir/
  cat input.mlir | %(prog)s --stdin -o output.ir
  %(prog)s input.mlir  # Print to stdout
    ...
    try:
        ...
        # Convert to XLS IR
        ir_code = mlir_to_ir(mlir_module)
        ...
        if args.output:
            ...
            if output_path.is_dir() or (not output_path.exists() and not output_path.suffix):
                ...
                output_file = output_path / f"{input_name}.ir"
            else:
            ...
            print(f"✓ Generated XLS IR: {output_file}")
        else:
            # Print to stdout
            print(ir_code)
